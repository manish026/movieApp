...
- (IBAction)searchClicked:(UIButton *)sender {
    ...
    if(![self.searchTextField.text  isEqual: @""] && [self connected]){
...
-(void)loadDataWithPage{
    ...
    if([self connected]){       // check if internet connection is available
...
-(void)loadSearchData{
    ...
    if(fromSearch){
        ...
        [self.parameterDictionary setValue:self.searchText forKey:@"query"]; // add require parameters to dictionary
        [self.parameterDictionary setValue:[NSString stringWithFormat:@"%lu",(unsigned long)page] forKey:@"page"];
        ...
        [self.parameterDictionary setValue:self.searchTextField.text forKey:@"query"]; // add require parameters to dictionary
